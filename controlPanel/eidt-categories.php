...
justify-content: center;
 ...
 $id = $_GET['id'];
 $sql = "Select * from maincategories where id = $id";
 $rs = $db->dbQuery($sql);
 $row = $db->dbFetchRecord($rs);
   if(isset($_POST['btnAdd'])){
     $title = $_POST['title'];
     
       $sql = "UPDATE `maincategories` SET `title` = '$title' WHERE `maincategories`.`id` = $id";
       $rs = $db->dbQuery($sql);
 ...
 <div class="row">
  <div class="col-sm-12">
    <div class="card-box">
      ...
      <h4 class="header-title m-t-0 m-b-30">تعديل تصنيف </h4>
        ...
        <div class="col-lg-6">
          <form class="form-horizontal" method="post" enctype="multipart/form-data">
                      <div class="form-group">
                          <label class="" style="display: block;text-align: center;">اسم المدينة</label>
                          <input type="text" class="form-control" value="<?= $row['title'] ?>"  name="title"  placeholder="اسم المدينة">
                      </div>
